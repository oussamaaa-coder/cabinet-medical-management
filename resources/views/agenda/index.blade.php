...
<div class="p-6 bg-gray-50 min-h-screen" x-data="calendarApp()" x-init="init()" x-cloak>
    <div class="flex flex-col md:flex-row gap-6">
        
        <!-- Left Column: Calendar -->
        <div class="w-full md:w-3/5 bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800" x-text="monthNames[month] + ' ' + year"></h2>
                    <p class="text-gray-500">Gérez vos rendez-vous</p>
                </div>
                <div class="flex gap-2">
                    <button @click="previousMonth()" class="p-2 hover:bg-gray-100 rounded-full transition-colors">
            ...
            <div class="calendar-grid gap-1">
                ...
                <template x-for="(date, dateIndex) in noofdays" :key="dateIndex">
                    <div 
                        ...
                        class="h-24 md:h-32 border border-gray-100 rounded-xl p-2 cursor-pointer transition-all hover:border-medical-blue relative group"
                        :class="{
                            'bg-medical-blue text-white shadow-lg shadow-green-100 border-medical-blue': isSelected(date),
                            'bg-green-50/50 border-green-200': isToday(date) && !isSelected(date)
                        }"
                    >
            ...
            <div class="flex flex-col gap-4 overflow-y-auto max-h-[calc(100vh-200px)] lg:pr-2">
                <template x-if="dailyAppointments.length === 0">
                    <div class="bg-white rounded-2xl border border-dashed border-gray-300 p-12 text-center">
                        ...
                        <h4 class="text-gray-500 font-medium">Aucun rendez-vous</h4>
                        <p class="text-sm text-gray-400">Pour ce jour sélectionné</p>
                    </div>
                </template>

                <template x-for="apt in dailyAppointments" :key="apt.id">
                    <div @click="viewDetails(apt)" class="bg-white border border-gray-200 rounded-2xl p-4 cursor-pointer hover:shadow-md transition-all border-l-4 group" :class="getStatusBorder(apt.status)">
                        <div class="flex justify-between items-start mb-2">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-medical-blue font-bold" x-text="apt.patient.name.substring(0,2).toUpperCase()"></div>
                                <div>
                                    <h4 class="font-bold text-gray-800" x-text="apt.patient.name"></h4>
                                    <p class="text-xs text-gray-500" x-text="apt.type"></p>
                                </div>
                            </div>
    ...
    <div x-show="showNewModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" x-transition.opacity>
        <div @click.away="showNewModal = false" class="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden">
            ...
            <form @submit.prevent="createAppointment" class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <textarea x-model="newApt.notes" rows="3" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-medical-blue focus:border-medical-blue outline-none transition-all" placeholder="Ajouter des notes..."></textarea>
                </div>
