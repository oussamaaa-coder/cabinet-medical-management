...
<div class="appointments-page-wrapper">
    <div class="app-topbar">
        <div class="app-breadcrumb">
            ...
            <span class="current">Planifiés aujourd'hui</span>
        </div>
    </div>
                        ...
                        <tr>
                            <td>
                                <div style="display: flex; align-items: center; gap: 12px; font-weight: 600; color: var(--text-primary);">
                                    <div style="width: 32px; height: 32px; border-radius: 8px; background: var(--accent-light); color: var(--accent); display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 700;">{{ strtoupper(substr($appointment->patient->first_name, 0, 1) . substr($appointment->patient->last_name, 0, 1)) }}</div>
                            ...
                            <td style="text-align: right;">
                                @php
                                    $statusClass = $appointment->status == 'urgent' ? 'background: #fee2e2; color: #ef4444;' : 'background: var(--accent-light); color: var(--accent);';
                                @endphp
                                <span class="badge app-badge" style="{{ $statusClass }}">
