...
<div class="testimonials-page-wrapper">
    ...
    <div class="app-topbar">
        <div class="app-breadcrumb">
            ...
            <span class="current">Modifier le témoignage</span>
        </div>
    </div>
    ...
    <div class="app-card" style="max-width: 680px; margin: 0 auto;">
        <div style="margin-bottom: 28px; border-bottom: 1px solid var(--border); padding-bottom: 20px;">
            <h3 style="margin: 0; font-size: 1.25rem; font-weight: 700; color: var(--text-primary);">Modifier le témoignage</h3>
            <p style="margin: 4px 0 0; font-size: 0.875rem; color: var(--text-muted);">Mettez à jour les informations de l'avis sélectionné.</p>
        </div>
        ...
        <form action="{{ route('testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="text" name="name" class="app-form-control" value="{{ old('name', $testimonial->name) }}" placeholder="ex: Sophie Martin" required>
                </div>
                <div>
                    <label class="app-form-label">Rôle / Titre</label>
                    <input type="text" name="role" class="app-form-control" value="{{ old('role', $testimonial->role) }}" placeholder="ex: Patiente depuis 2 ans">
                </div>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="content" class="app-form-control" rows="5" placeholder="Saisissez l'avis du patient ici..." required style="resize: vertical;">{{ old('content', $testimonial->content) }}</textarea>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                @if($testimonial->image)
                    <div style="margin-bottom: 12px; display: flex; align-items: center; gap: 12px;">
                        <img src="{{ asset($testimonial->image) }}" alt="Current" style="width: 60px; height: 60px; border-radius: 12px; object-fit: cover; border: 1px solid var(--border);">
                        <span style="font-size: 0.75rem; color: var(--text-muted);">Photo actuelle. Téléchargez-en une nouvelle pour la remplacer.</span>
                    </div>
                @endif
            ...
            <div style="margin-bottom: 28px; display: flex; align-items: center; gap: 10px;">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $testimonial->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
                <label for="is_active" style="font-size: 0.875rem; color: var(--text-primary); cursor: pointer; font-weight: 500;">Afficher sur le site</label>
            </div>
